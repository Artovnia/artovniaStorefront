...
import { translateError } from "@/lib/utils/error-messages";
...
const Form = ({
  ...
  const submit = async (data: FieldValues) => {
    ...
    // Priority 2: Redirect URL from query parameter
    if (redirectUrl) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      setError(
        translateError(error.message || "Google login failed")
      );
    }
  };
    ...
    >
      <div
        className={
          compact
            ? "space-y-4"
            : "w-96 max-w-full mx-auto space-y-4"
        }
      >
        <LabeledInput
          label="E-mail"
  ...
  return (
    <main className="container" aria-label="Logowanie">
