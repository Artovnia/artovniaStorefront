...
export const ImageZoomModal = ({
  ...
  const getImageKey = useCallback(
    (image: MedusaProductImage, index: number) => {
      return `${image.id ?? 'no-id'}-${image.url ?? 'no-url'}-${index}`
    },
    []
  )

  const prefetchAtIndex = useCallback(
    (index: number) => {
  ...
  const modalContent = (
    <div
      className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-sm min-h-screen"
    ...
    >
      ...
      <div className="absolute top-0 left-0 right-0 z-10 flex justify-between items-center p-4 bg-gradient-to-b from-black/50 to-transparent">
        ...
        <button
          onClick={onClose}
          className="text-white hover:text-gray-300 p-2 hover:bg-white/10 rounded-full"
      ...
      {images.length > 1 && (
        <>
          <button
            ...
            className="absolute left-4 top-1/2 -translate-y-1/2 z-10 text-white hover:text-gray-300 p-3 hover:bg-white/10 rounded-full"
          ...
          <button
            onClick={goToNext}
            onMouseEnter={() => prefetchAtIndex(currentIndex + 1)}
            className="absolute right-4 top-1/2 -translate-y-1/2 z-10 text-white hover:text-gray-300 p-3 hover:bg-white/10 rounded-full"
        ...
        <div className="absolute bottom-16 left-1/2 -translate-x-1/2 flex gap-2 bg-black/30 p-3 rounded-lg backdrop-blur-sm">
          {images.map((image, index) => (
            <button
              ...
              className={`relative w-12 h-12 rounded-sm overflow-hidden border-2 ${
                currentIndex === index
