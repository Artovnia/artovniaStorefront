...
export default async function SellerPostPage({ params }: SellerPostPageProps) {
  ...
  const [post, categories] = await Promise.all([getSellerPost(slug), getBlogCategories()])
  ...
  try {
    mainImageUrl = post.mainImage ? urlFor(post.mainImage).width(800).height(640).url() : null
    secondaryImageUrl = post.secondaryImage ? urlFor(post.secondaryImage).width(400).height(300).url() : null
  } catch {
    // Ignore
  }
      ...
      <article className="bg-[#F4F0EB]" itemScope itemType="https://schema.org/ProfilePage">
          ...
          <div className="relative z-10 max-w-7xl mx-auto px-4 lg:px-8 py-12">
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 items-center">
              <header className="space-y-6 lg:space-y-8">
                ...
                <p className="text-lg md:text-xl text-[#3B3634] leading-relaxed font-light" itemProp="description">
                ...
                <Link
                  href={post.sellerUrl}
                  className="inline-flex items-center space-x-3 ring-1 ring-[#BFB7AD] backdrop-blur-[1px] text-black px-6 md:px-8 py-3 md:py-4 hover:bg-[#3B3634] hover:text-white transition-all duration-300 group"
        ...
        <section className="py-12 md:py-16 lg:py-24">
          <div className="max-w-4xl mx-auto px-4 lg:px-8">
            ...
            >
              {post.content ? <PortableText content={post.content} /> : <p>Brak treści artykułu.</p>}
            </div>

            {post.linkedProducts && post.linkedProducts.length > 0 && (
              <aside
                className="mt-12 md:mt-16 pt-12 md:pt-16 border-t border-[#BFB7AD]/30"
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8" role="list">
                  {post.linkedProducts.map((product, index) => {
                    ...
                    return (
                      ...
                      >
                        <Link href={productUrl} className="block" aria-label={`Zobacz produkt: ${product.productName}`}>
