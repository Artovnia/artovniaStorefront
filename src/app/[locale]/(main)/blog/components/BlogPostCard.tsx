...
export default function BlogPostCard({
  ...
  // === IMAGE SOURCE DIMENSIONS — adjust width/height here to change fetched resolution ===
  ...
  return (
    ...
    >
      <article className="flex flex-col h-full w-full">
        {/* === IMAGE CONTAINER HEIGHT — adjust h-80/h-64 (featured/regular) to resize image === */}
        ...
        <div className="p-4 pb-10 flex-1 flex flex-col">
            ...
            <div className="flex flex-wrap gap-2 mb-3">
              {post.categories.map((category) => (
                <span
                  ...
                  className="text-xs font-medium bg-blue-50 px-2 py-1 rounded font-instrument-sans"
                >
                  {category.title}
                </span>
              ))}
          ...
          <h3
            className={`font-normal text-[#3B3634] transition-colors duration-200 mb-2 font-instrument-serif line-clamp-2 ${featured ? "text-2xl" : "text-xl"}`}
          >
            {post.title}
          </h3>
          ...
          {post.tags && post.tags.length > 0 && (
            <div className="flex flex-wrap gap-1 mt-auto pt-4">
              ...
              {post.tags.length > 3 && (
                <span className="text-xs py-1 text-gray-500 font-instrument-sans">
